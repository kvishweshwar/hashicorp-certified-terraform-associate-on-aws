...
resource "aws_instance" "my-ec2-instance" {
  ...
  // While using availability_zone with random_shuffle; we can not guarantee about assigning correct subnet_id. Hence, disabled.
...
module "ec2_cluster" {
  ...
  key_name               = "awsuser-key"
  user_data              = file("install-httpd.sh")
  availability_zone      = element(aws_subnet.my-vpc-public-subnet[*].availability_zone, count.index > 5 ? count.index % length(data.aws_availability_zones.aws-availability-zones.names) : count.index)
  subnet_id              = element(aws_subnet.my-vpc-public-subnet[*].id, count.index > 5 ? count.index % length(data.aws_availability_zones.aws-availability-zones.names) : count.index)
  vpc_security_group_ids = [aws_security_group.my-vpc-default-sg.id, aws_security_group.my-vpc-web-application-sg.id]

  tags = {
    created_using = "module:ec2_cluster"
  }
}
...
resource "null_resource" "null-resource" {
  ...
  connection {
    type        = "ssh"
    host        = element(aws_instance.my-ec2-instance[*].public_ip, count.index)
    user        = "ec2-user"
    password    = ""
    private_key = file("./sshkeys/awsuser-key.pem")
  }
  provisioner "file" {
    source      = "./webapps/index.html"
    destination = "/tmp/index.html"
  }
  provisioner "remote-exec" {
    inline = [
      "sudo cp -r /tmp/index.html /var/www/html/index.html"
    ]
  }
}
*/
